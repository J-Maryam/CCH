...
public class CyclistServiceImplTest {
    ...
    @Test
    public void testFindAllSortedByLName_SingleCyclist() {
        Cyclist cyclist = new Cyclist("John", "Doe", "USA", LocalDate.of(1990, 1, 1), new Team("Team A"));
        when(cyclistRepository.findAllSortedBylName("Doe")).thenReturn(Collections.singletonList(cyclist));

        List<Cyclist> sortedCyclists = cyclistService.findAllSortedByLastName("Doe");

        assertEquals(1, sortedCyclists.size());
        assertEquals("Doe", sortedCyclists.get(0).getLName());
    }

    @Test
    public void testFindAllSortedByTeam_SameTeam() {
        Team team = new Team("Team A");
        Cyclist cyclist1 = new Cyclist("John", "Doe", "USA", LocalDate.of(1990, 1, 1), team);
        Cyclist cyclist2 = new Cyclist("Jane", "Smith", "Canada", LocalDate.of(1992, 5, 15), team);

        List<Cyclist> cyclists = Arrays.asList(cyclist1, cyclist2);
        when(cyclistRepository.findAllSortedByTeam(team)).thenReturn(cyclists);

        List<Cyclist> sortedCyclists = cyclistService.findAllSortedByTeam(team);

        assertEquals("Team A", sortedCyclists.get(0).getTeam().getTeam());
        assertEquals("Team A", sortedCyclists.get(1).getTeam().getTeam());
    }
